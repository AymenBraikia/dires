...
class Rain_System {
	...
	draw() {
		...
		for (let i = 0; i < this.drops.length; i++) {
			...
			drop.opacity = drop.y / user.screenH! + 0.1;
	...
	init(ctx: CanvasRenderingContext2D) {
		...
		for (let i = 0; i < this.max_drops; i++) this.drops.push(new rain_drop(Math.random() * (user.screenW! * 1.1) - user.screenW! * 0.1, 0, 1, Math.random() * 4 + 4));
	}
}
...
export const rain = new Rain_System();
